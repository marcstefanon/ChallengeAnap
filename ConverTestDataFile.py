...
FileLoc = FilePath + csv_files[2]; print(FileLoc)
...
for row in data:
   ...
   temp = row[0].split(';');
   Finess.append(temp[1]); Origin.append(temp[3][0:3]); Speciality.append(temp[4][1:3]);
   Age.append(temp[5]); SejourALD.append(temp[6]); Sejourtot.append(temp[7]);
   Year.append(temp[8]); 
...
FileLoc = FilePath + 'DataTest'
np.savez_compressed(FileLoc, Finess=Finess, Origin=Origin, Speciality=Speciality, Age=Age,SejourALD=SejourALD,Sejourtot=Sejourtot,Year=Year)
